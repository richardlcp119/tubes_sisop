def hitung_priority(arrival_times, burst_times, priorities):
    ...
    for i in range(n):
        proses.append({
            ...
            'priority': priorities[i], # Atribut tambahan untuk Priority Scheduling
            'is_completed': False
        })
    ...
    while selesai < n:
        ...
        if eligible:
            # Prinsip Priority: Urutkan berdasarkan priority terkecil (angka lebih kecil = prioritas tinggi). 
            # Jika priority sama, prioritaskan arrival_time terkecil.
            eligible.sort(key=lambda x: (x['priority'], x['arrival_time']))
            p = eligible[0] # Ambil proses prioritas tertinggi
